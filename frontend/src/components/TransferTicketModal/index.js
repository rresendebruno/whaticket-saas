...
const TransferTicketModal = ({ modalOpen, onClose, ticketid, ticketWhatsappId }) => {
	...
	useEffect(() => {
		const loadQueues = async () => {
			...
			const filtered = list.filter(q => !q.isGroupQueue);
			setAllQueues(filtered);
			setQueues(filtered);
		}
		loadQueues();
	...
	const handleSaveTicket = async e => {
		...
		if (!selectedUser && !selectedQueue) {
			toastError({ response: { data: { message: "Selecione um agente ou uma fila para transferir." } } });
			return;
		}
		setLoading(true);
		try {
			...
			await api.put(`/tickets/${ticketid}`, { ...data, isTransfer: true });
